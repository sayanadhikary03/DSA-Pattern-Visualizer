...
import { Layers, RefreshCw, GitCommit, Database, GitMerge, Share2, Award, ArrowRight } from 'lucide-react';
...
const categories = [
  {
    ...
    icon: Database,
    description: 'Master sliding window, two pointers, and prefix sum patterns on linear blocks of memory.',
    preview: (
  ...
  },
  {
    ...
    icon: RefreshCw,
    description: 'Visualize Bubble, Quick, Merge, and Heap sorts in real-time, watching items find their positions.',
    preview: (
      <div className="flex items-end justify-center gap-2 h-14 my-2">
        {[30, 60, 45, 90, 75, 50].map((h, i) => (
          <motion.div
            ...
            initial={{ height: 10 }}
            animate={{ height: `${h}%` }}
            transition={{ repeat: Infinity, repeatType: 'reverse', duration: 1.5, delay: i * 0.15 }}
  ...
  },
  {
    ...
    icon: GitCommit,
    description: 'Understand node linkages, recursive pointers, fast & slow pointers, and cycle detection.',
    preview: (
  ...
  },
  {
    ...
    icon: Layers,
    description: 'Track Last-In-First-Out (LIFO) and First-In-First-Out (FIFO) push/pop operations visually.',
    preview: (
  ...
  },
  {
    ...
    icon: GitMerge,
    description: 'Binary trees, BFS/DFS traversal order, and AVL balancing step-by-step.',
    preview: (
      <div className="relative w-full h-12 flex items-center justify-center my-3">
        <div className="w-6 h-6 rounded-full bg-zinc-900 border border-zinc-700 flex items-center justify-center text-white text-[9px] z-10">R</div>
        <div className="absolute top-7 left-12 w-6 h-6 rounded-full bg-sky-600 flex items-center justify-center text-white text-[9px] z-10">L</div>
        <div className="absolute top-7 right-12 w-6 h-6 rounded-full bg-sky-600 flex items-center justify-center text-white text-[9px] z-10">R</div>
        <svg className="absolute inset-0 w-full h-full">
          <line x1="50%" y1="12" x2="35%" y2="40%" stroke="#cbd5e1" strokeWidth="2" />
          <line x1="50%" y1="12" x2="65%" y2="40%" stroke="#cbd5e1" strokeWidth="2" />
        </svg>
      </div>
    ),
  },
  {
    ...
    icon: Share2,
    description: 'Navigate nodes and edges via Dijkstra, BFS, DFS, and topological sorts.',
    preview: (
      <div className="relative w-full h-12 flex items-center justify-center gap-6 my-3">
        ...
        <svg className="absolute inset-0 w-full h-full">
          <line x1="30%" y1="24" x2="50%" y2="24" stroke="#c084fc" strokeWidth="2" />
          <line x1="50%" y1="24" x2="70%" y2="24" stroke="#c084fc" strokeWidth="2" />
        </svg>
      </div>
    ),
  },
  {
    ...
    icon: Award,
    description: 'Deconstruct subproblems, build memoization tables, and watch DP matrices fill.',
    preview: (
      <div className="grid grid-cols-3 gap-0.5 max-w-[90px] mx-auto my-3 border border-zinc-300 rounded p-0.5 bg-white">
        {Array.from({ length: 9 }).map((_, i) => (
          <motion.div
            key={i}
            initial={{ bg: 'white' }}
            animate={{ backgroundColor: i % 4 === 0 ? '#f97316' : '#ffffff', color: i % 4 === 0 ? '#ffffff' : '#121214' }}
...
export default function CategoryPreview() {
  return (
    <section id="categories" className="py-20 px-6 md:px-12 bg-[#FCF9F3] border-t border-[#ebdcb9]/40">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-end justify-between mb-16 gap-4">
          ...
          <p className="font-sans font-medium text-sm md:text-base text-zinc-500 max-w-md">
            Click into any category to explore structured code execution visualizers tailored for core interview pattern schemas.
          </p>
        </div>
          ...
          {categories.map((cat, idx) => {
            ...
            return (
              ...
              >
                <Link
                  to="/patterns"
                  className={`group block h-full p-6 md:p-8 rounded-3xl border-3 border-zinc-900 shadow-[6px_6px_0px_rgba(18,18,20,1)] hover:shadow-[10px_10px_0px_rgba(18,18,20,1)] hover:-translate-x-0.5 hover:-translate-y-0.5 transition-all duration-300 flex flex-col justify-between ${cat.color}`}
                >
                  <div>
                    {/* Header: ID + Icon */}
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-2 text-xs font-space font-bold tracking-wider text-zinc-900 uppercase mt-4 pt-4 border-t border-zinc-900/10">
                      Explore patterns
                      <ArrowRight size={13} className="transition-transform duration-300 group-hover:translate-x-1 text-indigo-600" />
                    </div>
                  </div>
                </Link>
              </motion.div>
